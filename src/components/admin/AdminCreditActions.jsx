...
export default function AdminCreditActions({ businessId, currentBalance }) {
  ...
  const handleAdjust = async (amount) => {
    const reason = window.prompt(`Reason for ${amount > 0 ? 'adding' : 'removing'} ${Math.abs(amount)} Voxy Points (VP):`, 'Manual Admin Adjustment');
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <button 
        onClick={() => {
          const val = window.prompt('Enter exact amount of VP to add (positive) or remove (negative):');
          if (val && !isNaN(val)) handleAdjust(parseInt(val));
        }}
        disabled={loading}
        title="Exact VP Amount"
